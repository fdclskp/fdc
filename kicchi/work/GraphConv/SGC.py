...
class SGC(Chain):
	...
	def __call__(self, x, stock_data):

		def read_features(self, graphs):
			adj_list = []
			code_features = []
			for i in range(len(graphs)):
				adj_list.append(graphs[i].nodes)
			for i in range(len(graphs)):
				code_feature = []
				for j in range(len(adj_list[i])):
					code_feature.append(stock_data[adj_list[i].tolist()[j][0]])
		...
		def daily_features_func(self, graphs):
			...
			for layer in range(self.params['gnn_layers']):
				ret = []
				for i in range(len(code_features)):
					ret.append(update_layer(self, code_features[i], adj_lists[i]))
				code_features = np.array(ret)
			return code_features
		daily_features = daily_features_func(self,x)
