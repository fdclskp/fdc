...
class SGC(Chain):
	...
	def __call__(self, x, stock_data, params):
		...
		def read_features(self, graph):
			adj_list = []
			code_features = []
			for i in range(len(graph)):
				adj_list.append(graph[i].adj_list)
			for i in range(len(graph)):
				code_feature = []
				for j in range(len(adj_list[i])):
					code_feature.append(stock_data[str(adj_list[i][j])])
		...
		def daily_features_func(self, graphs):
			code_features, adj_list= read_features(self, graphs)
			for layer in range(params['gnn_layers']):
				ret = []
				for i in range(len(code_features)):
					code_features = update_layer(self, code_features, adj_list)
